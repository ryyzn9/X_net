...
class MultiheadAttn(nn.Module):
    def __init__(self,
        ...
        assert d_model%head_num == 0, f"d_model is not divisible by h where h is {head_num}"
    ...
    def attention(query, 
        ...
        #(batch,head_num,seq_len,dk)-->(batch,head_num,seq_len,seq_len)
        attention_score =(query @ key.transpose(-2,-1))/ math.sqrt(d_k)
        if mask is not None:
            # write a very low value (inicating - inf ) to the position where mask is zero 
            attention_score.masked_fill_(mask ==0, -1e9)

        attention_score = attention_score.softmax(dim=-1) #(batch,head,seq_len,seq_len)# Apply softmax



        if dropout is not None:
            attention_score = dropout(attention_score)
            #(batch,head_num , seq_len,seq_len )--> (batch,head_num,seq_len,dk)
            #  return extra attetion score as it can be used for visualization
    ...
    def forward(self,q,k,v,mask):
        ...
        x , self.attention_scores = MultiheadAttn.attention(query,key ,value ,mask,self.dropout) 
...
class EncoderBlock(nn.Module):
    ...
    def forward(self,x,src_mask):
        x = self.resnet[0](x ,lambda x:self.MHAttention(x,x,x,src_mask))
...
class DecoderBlock(nn.Module):
    ...
    def forward(self,x,encoder_output,src_mask,tgt_mask):
        x = self.resnet[0](x,lambda x : self.attention(x,x,x,tgt_mask))
        x = self.resnet[1](x, lambda x : self.crossAttenion(x,encoder_output,encoder_output,src_mask)) 
...
class Decoder(nn.Module):
    def __init__(self,features:int,layers:nn.ModuleList) -> None:
        ...
        self.layers = layers
        self.norm = LayerNorm(features)
    def forward(self,x,encoder_output,src_mask,tgt_mask)->None:
...
class Transformer(nn.Module):
    def __init__(self,encoder:Encoder,
                 ...
                 encoder_pos:PositioalEncoding,
                 decoder_pos:PositioalEncoding,
        ...
        self.en_pos = encoder_pos
        self.de_pos = decoder_pos
...
def build_transformer(en_vocab_size:int,
    ...
    encoder_pos = PositioalEncoding(d_model,src_seq_len,dropout)
    decoder_pos = PositioalEncoding(d_model,tgt_seq_len,dropout)
    ...
    transformer1 = Transformer(encoder,decoder,en_embed,de_embed,encoder_pos,decoder_pos,projection_layer)  
